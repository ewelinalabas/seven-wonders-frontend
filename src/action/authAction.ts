...
export const signIn = (data): AppThunk<Promise<any>> => (dispatch, getState, { authApi }) =>
  dispatch(async(SIGN_IN, authApi.signIn(data))).then(() => dispatch(redirectTo(AppRoute.HOME)));

export const SIGN_UP = 'SIGN_UP';
export const signUp = (data): AppThunk<Promise<any>> => (dispatch, getState, { authApi }) =>
  dispatch(async(SIGN_UP, authApi.signUp(data))).then(() => dispatch(redirectTo(AppRoute.HOME)));
